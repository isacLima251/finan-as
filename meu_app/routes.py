...
from meu_app.models import Gasto, Pedido
...
def painel():
    ...
    if termo_busca:
        ...
        query = query.filter(
            or_(
                Pedido.cliente.ilike(like_termo),
                Pedido.telefone.ilike(like_termo),
            )
        )
    ...
    if periodo_selecionado == 'personalizado' and data_inicio_str and data_fim_str:
        ...
        data_inicio = datetime.combine(
            primeiro_dia_mes_passado, datetime.min.time()
        )
    ...
    total_agendado = (
        ...
        .filter(Pedido.status == 'Agendado', Pedido.data_venda.between(data_inicio, data_fim))
        .scalar()
        or 0.0
    )
    total_pago = (
        db.session.query(func.sum(Pedido.valor))
        .filter(Pedido.status == 'Pago', Pedido.data_pagamento.between(data_inicio, data_fim))
        .scalar()
        or 0.0
    )
    total_frustrado = (
        db.session.query(func.sum(Pedido.valor))
        .filter(Pedido.status == 'Frustrado', Pedido.data_venda.between(data_inicio, data_fim))
        .scalar()
        or 0.0
    )
    total_gasto = (
        db.session.query(func.sum(Gasto.valor))
        .filter(Gasto.data.between(data_inicio, data_fim))
        .scalar()
        or 0.0
    )
    total_a_receber = (
