...
def painel():
    ...
    if periodo_selecionado == 'personalizado' and data_inicio_str and data_fim_str:
        data_inicio = datetime.strptime(data_inicio_str, '%Y-%m-%d'); data_fim = datetime.combine(datetime.strptime(data_fim_str, '%Y-%m-%d'), datetime.max.time())
    ...
    else: primeiro_dia_mes_atual = hoje.replace(day=1); data_inicio = datetime.combine(primeiro_dia_mes_atual, datetime.min.time()); data_fim = datetime.combine(hoje, datetime.max.time())

    total_agendado = db.session.query(func.sum(Pedido.valor)).filter(Pedido.status == 'Agendado', Pedido.data_venda.between(data_inicio, data_fim)).scalar() or 0.0
    total_pago = db.session.query(func.sum(Pedido.valor)).filter(Pedido.status == 'Pago', Pedido.data_pagamento.between(data_inicio, data_fim)).scalar() or 0.0
    total_frustrado = db.session.query(func.sum(Pedido.valor)).filter(Pedido.status == 'Frustrado', Pedido.data_venda.between(data_inicio, data_fim)).scalar() or 0.0
    total_gasto = db.session.query(func.sum(Gasto.valor)).filter(Gasto.data.between(data_inicio, data_fim)).scalar() or 0.0
    ...
    resumo_dados = { 'agendado': total_agendado, 'pagos': total_pago, 'a_receber': total_a_receber, 'frustrados': total_frustrado, 'atrasados': total_atrasado, 'gastos': total_gasto, 'lucro': total_pago - total_gasto, 'projecao': total_pago + total_a_receber + total_agendado }
